	...
	function init_addevent(param){
		...
		for(var i = 0; i < 2; i++){
			$("#addevent_div_" + name[i]).css("display",param["body"][i]["display"]);
			$("#addevent_div_" + name[i] + " span").html(param["body"][i]["name"]);
			$("#addevent_" + name[i]).val(param["body"][i]["val"]);
			$("#addevent_" + name[i]).attr("placeholder",param["body"][i]["placeholder"]);
		...
		//set footer
		$("#addevent .modal-footer button.btn-primary").attr("onclick",param["footer"]);
	}
