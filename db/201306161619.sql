...
ALTER TABLE `messagefk`.`no_finish` DROP FOREIGN KEY `fk_no_finish_problem1` ;

ALTER TABLE `messagefk`.`user` DROP COLUMN `name` , CHANGE COLUMN `lev` `lev` INT(11) NOT NULL COMMENT '0 normal\\n1 fixCenter\\n2 roomCenter\\n3 admin\\n'  AFTER `email` , CHANGE COLUMN `phone` `phone` VARCHAR(11) NULL DEFAULT NULL  ;
...
ALTER TABLE `messagefk`.`no_finish` ADD COLUMN `no_finishcol` VARCHAR(45) NULL DEFAULT NULL  AFTER `send_message_time` 
, DROP INDEX `fk_no_finish_problem1_idx` ;
...
DROP TABLE IF EXISTS `messagefk`.`message` ;
